...
- **Persistence (R2)** — an R2 bucket (`comfyui`) is the persistence layer:
  ...
  - **User dir** (`r2://comfyui/user`): workflows, `__manager/config.ini`, comfy
    ...
    so deletions don't propagate (prune R2 manually). Excludes `__manager/cache`,
    `*.log`, and `comfyui.db*` (a live SQLite DB — unsafe to file-copy, and
    regenerated by ComfyUI anyway).
